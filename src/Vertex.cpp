...
namespace {
constexpr bool MAX_BAD_EQ =
    (POLY_Dmax > 5); /* previously 6; needed for nef !? */
constexpr bool SHOW_NEW_CEq =
    false; /* (POLY_Dmax>12) tracks polytope analysis */
} // namespace
...
constexpr bool VERT_WITH_MAX_DISTANCE =
    false;                            /* 0 @ [1845 2 15 97 247 610 874]    */
...
int Ref_Check(PolyPointList *_P, VertexNumList *_V, EqList *_F) {
  ...
  _F->ne = 0;
  i = Finish_REF_Check(_P, _V, _F, CEq.get(), F_I.data(), CEq_I.data());
  return i;
}
...
void RC_Calc_BaHo(PolyPointList *_P, VertexNumList *_V, EqList *_E,
  ...
  auto _FI = std::make_unique<FaceInfo>();
  _BH->mp = _P->np;
  _BH->mv = _V->nv;
  _BH->nv = _E->ne;
  _BH->np = _DP->np;
