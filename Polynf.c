...
{    int i,j; Long g, a, b, x[POLY_Dmax+1], *X=W[*nw]; 
	...
	if(b<=0) return 0; else g=Fgcd(a,b);
	x[j]=b/g; 
...
{    Long P[VERT_Nmax][POLY_Dmax],F[POLY_Dmax][VERT_Nmax],B[POLY_Dmax],g,
	...
	for(i=0;i<D;i++) B[i]=Ne*B[i]+Ve*(*_N)*ZB[i];
	(*_N) *= (Ne*g);
     }
     ...
     for(i=0;i<D;i++) g=NNgcd(g,_B[i]);
     for(i=0;i<D;i++) _B[i] /=g;
     (*_N) /=g; 
     return vol;
}
...
int SemiSimpleRoots(PolyPointList *P,EqList *E,Long **R){
  ...
  for(p=0;p<P->np;p++){
    int z=0;
    for(e=0;e<E->ne;e++) if(0==Eval_Eq_on_V(&E->e[e],P->x[p],P->n)) z++;
    if(z==1) R[N++]=P->x[p];}
  if(N%2) return 0;
  if(N==0) return -1; 
  ...
  /*for(p=0;p<N;p++){for(e=0;e<N;e++)if(e!=p)if(ZeroSum(R[p],R[e],d)) break;
      if(e==N)return 0;}*/
  for(p=0;p<N;p++){
    for(e=p+1;e<N;e++) if(ZeroSum(R[p],R[e],d)) break;
    if(e==N) return 0;
    if(e>p+1){Long *X=R[e];R[e]=R[p+1];R[p+1]=X;}
    p++;}
  return N;}
...
Long V_to_GLZ(Long *V,Matrix G){          /* V may contain NULLs, return gcd */
  ...
  if(p>1){ g=W_to_GLZ(W,&p,G.x); if(g<0) for(i=0;i<p;i++) G.x[0][i]*= -1; i=p; 
    ...
    else if(*V<0) G.x[0][0]=-1;
    g=V[P[0]]; }
  if(g<0) g=-g;
  for(j=0;j<d;j++) assert(VxV(V,G.x[j],d)==((j==0)*g));
  free(Z); 
  return g;}
...
void Aux_MinNonNeg_UT(Matrix M,Matrix G,int c,int r,int d,Long D){
  int i,j; for(i=0;i<r;i++){Long X=VxV(G.x[i],M.x[c],d), R=X/D; 
    if(X-R*D<0)R-=1;
    for(j=0;j<d;j++)G.x[i][j]-=R*G.x[r][j];}}
 ...
 */
	...
	if(e==4){
		...
		if(X[0][i]+X[2][i]-X[1][i]-X[3][i]) break;
	      if(i==P->n)sq=2;}
	    if(!sq){for(i=0;i<P->n;i++)
		if(X[0][i]+X[3][i]-X[1][i]-X[2][i]) break;
	      if(i==P->n)sq=3;}
...
{    int i,j,tot=0,reg=0,sym=0,ksum=0,sum=0,bary=0,ssroot=0,nofip=0,NR=NON_REF;
	...
	LatVol_Barycent(P,V,C,&N);bcz=(N==0);if(bcz)bary++;/*BaryCenterZero*/
	    ...
	    for(i=0;i<V->nv;i++) kPM[j][i]=x*PM[j][i];
	    E->e[j].c=x; 
	...
	{   int nr=(ssr+1) ? ssr : 0; printf("%d %d    v=%d p=%d roots=%d  "
	    ,P->n,V->nv+nr,V->nv,P->np,nr); puts(c);
	  for(i=0;i<P->n;i++){
	    for(j=0;j<V->nv;j++) printf("%2ld ",P->x[V->v[j]][i]);
	    printf("  ");
	    for(j=0;j<nr;j++) printf(" %2ld",root[j][i]);
	    puts("");
	}   /* Print_PPL(P,c); */ }
...
{    int n, i, j, d=P->n, s[POLY_Dmax];
	...
	*GI[POLY_Dmax][POLY_Dmax], GX[(POLY_Dmax*(POLY_Dmax+3))/2][POLY_Dmax];
	 ...
	 if(i<n) GI[n][i]=GI[n-1][i]; else GI[n][i]=GX[j++];
       G[n]=GI[n];}
...
{    int s[VERT_Nmax],t[VERT_Nmax],d=P->n,p=P->np-1; PolyPointList *A; 
	...
	for(i=0;i<p;i++) if(t[i]==0)s[j++]=i;
	e=j;
	for(i=0;i<p;i++) if(t[i]==1)s[j++]=i;
	k=j;
	for(i=0;i<p;i++) if(t[i]==2)s[j++]=i;
...
{    int x,s[VERT_Nmax],t[VERT_Nmax],d=P->n,p=P->np-1; PolyPointList *A=NULL; 
	...
	{   fprintf(outFILE,"  p="); Print_Perm(s,p,"\n");
        ...
        for(i=0;i<p;i++) for(j=0;j<d;j++) A->x[i][j]=PM[s[i]][j];
	A->n=d;A->np=p;
	Make_Poly_UTriang(A);
	for(j=0;j<d;j++)for(i=0;i<p;i++) (P->np>20) ? 
...
{    int x, fdim=P->n-cd; ek3fli *F = (ek3fli *) malloc ( sizeof(ek3fli) ); 
	...
	{   for(j=0;j<d;j++)PM[i][j]=GxP(F->G[x][j],P->x[i],&d);
	    while(!PM[i][--j]) if(j==0) break;
	    if(j<fdim) t[i]=0; else t[i]=1;
	}   j=0; 
	for(i=0;i<p;i++) if(t[i]==0) s[j++]=i;
	fn=j;
	for(i=0;i<p;i++) if(t[i]==1) s[j++]=i;
	fprintf(outFILE,"%d %d  ",d,p); 
        for(i=0;i<fn;i++) for(j=0;j<fdim;j++)A->x[i][j]=PM[s[i]][j];
	A->n=fdim;
...
{    int n,i,j, s[POLY_Dmax+1]; int *nw=&F->nw;     /* nw=#IP_Simp <= Wmax */
	...
	*GI[POLY_Dmax][POLY_Dmax], GX[(POLY_Dmax*(POLY_Dmax+3))/2][POLY_Dmax];
	...
	for(j=0;j<p;j++) printf(" %ld",F->W[i][j]);
	puts("");exit(0);}}
}
...
{    int j; if(VS) Print_VL(_P,_V,"vertices of P-dual and IP-simplices");
     ...
     else { fprintf(outFILE,"         #=%d",nw);
       if(nw>np-_P->n) fprintf(outFILE," > %d=#pts-dim",np-_P->n);
       fputs("\n",outFILE);}
}
...
{    int n; char C[VERT_Nmax]; VertexNumList V; EqList E; for(n=0;n<F->nf;n++)
	...
	for(i=0;i<P->np-1;i++) C[i]='_';
	for(i=0;i<c;i++)C[s[i]]='p'; 
	for(i=0;i<V.nv;i++) C[s[V.v[i]]]='v';
	for(i=0;i<P->np-1;i++) 
...
{    int i,j,CD=0,np=_P->np-1; FibW *F = (FibW *) malloc( sizeof(FibW) );
	...
	{   EqList E; assert(Ref_Check(_P,&V,&E)); nv=V.nv-1;
	    for(i=0;i<nv;i++) for(j=i+1;j<V.nv;j++)
				if(V.v[i]>V.v[j]) swap(&V.v[i],&V.v[j]);
	    nv=V.v[V.nv-1]+1;
	}
     }
     if(VS)
	...
	{   for(i=0; i<np; i++) fprintf(outFILE,(np>20) ? "---" : "-----");
	    ...
	    if(F->nw>np-_P->n) fprintf(outFILE," > %d=#pts-dim",np-_P->n);
	    fputs("\n",outFILE);
	    for(i=0; i<F->nw; i++)
...
{    Long g=labs(GxP(G[l],X,d)), x; 
     while(++l<*d) if((x=labs(GxP(G[l],X,d)))) g=(g) ? Fgcd(g,x) : x;
     return g;
}
...
{    int L,C;Long g,a,index=1;GL_Long V[POLY_Dmax],*G[POLY_Dmax],*B[POLY_Dmax];
     ...
     for(L=0;L<d;L++) for(C=0;C<d;C++)G[L][C]=(L==C);
     for(L=0;L<d-1;L++)
...
{    int i,j,I; GL_Long GLG[POLY_Dmax][POLY_Dmax], Ginv[POLY_Dmax][POLY_Dmax];
     ...
     for(i=0;i<d;i++) for(j=0;j<d;j++) G[i][j]=GLG[i][j];
     return I;
}
...
{    int i,j; fprintf(stderr,"Z-action:\n");
     for(i=0;i<*r;i++){
       for(j=0;j<*p;j++) fprintf(stderr,"%5ld ",Z[i][j]);
       fprintf(stderr,"  /Z%ld\n",M[i]);}
}
...
Long AuxLinRelGPZ(Long *A,int *j,Long *D,int *d,Long GP[][POLY_Dmax],int *p,
	Long Z[][VERT_Nmax])
{    Long s=0,g=0; int i,l;
  for(l=0;l<*p;l++){
    A[l]=(l==(*j));
    for(i=0;i<*d;i++) A[l]-=(GP[*j][i]/D[i])*Z[i][l];
    g=(g) ? NNgcd(A[l],g) : A[l];}
...
{    int i,j; for(i=0;i<r;i++) if(Phase(Z[i],p)%M[i]) break; if(i==r) return;
     ...
     for(j=0;j<p;j++) fprintf(stderr," %ld",Z[i][j]);
...
{    Long g,a,index=1,*Y; GL_Long V[POLY_Dmax], *G[POLY_Dmax], *B[POLY_Dmax]; 
	...
	{   int c,l; Long vg,va; for(C=0;C<p;C++) GP[C][L]=GxP(G[L],P[C],&d); 
	    ...
	    g=GP[C][L]; for(c=0;c<p;c++) A[c]=0; A[C]=1;
	    for(c=C+1;c<p;c++)
	    {	if((a=GP[c][L]) % g) {
		g = Egcd(g,a,&vg,&va);
		for(l=C;l<p;l++) A[l]*=vg;
		A[c]+=va;}
	    }
	    if(g<0){g=-g; for(c=0;c<p;c++) A[c]*=-1;}
	    index*=D[L]=g; for(c=0;c<p;c++) Y[c]=A[c];
	    for(l=0;l<L;l++)
	...
	for(j=0;j<p;j++) fprintf(stderr," %4ld",P[j][i]);
	fprintf(stderr,"\n");}
	for(i=0;i<d;i++){
	  fprintf(stderr,"Z%d:  ",i);
	  for(j=0;j<p;j++) fprintf(stderr," %4ld",Z[i][j]);
	  fprintf(stderr,"  -> %ld /%ld\n", Phase(Z[i],p)%D[i],D[i]);}
	for(i=0;i<d;i++){fprintf(stderr,"GP: ");
	  for(j=0;j<p;j++) fprintf(stderr,"%5ld",GP[j][i]);
	  fprintf(stderr,"\n");}
	fprintf(stderr,"\n"); exit(0);}
	...
	{   g=0;
	  for(C=0;C<p;C++) if((a=labs(GxP(G[L],P[C],&d)))) 
		g = (g) ? Fgcd(g,a) : a;
	  index *= D[L]=g;
	  if(index!=1){
...
{    int i,j, N=W->N,w=W->nw, nz=0,np=0, z[AMBI_Dmax], p[AMBI_Dmax],
	pi[AMBI_Dmax], X[AMBI_Dmax]; Long *L=W->W[0];
     ...
     for(i=0;i<np;i++) pi[i]=p[i];
     for(i=0;i<nz;i++) pi[np+i]=z[i];
...
{    int i,j,r=0,J; Long g; for(i=0;i<*d;i++) if(W[i]) Waux[r++]=W[i];
	...
	if(t!=g*(i==0)) break;}
     if(i<*d) {
       ...
       for(i=0;i<*d;i++) fprintf(stderr,"%ld ",W[i]);
       fprintf(stderr,"=W  nonzero=%d<%d\n",r,*d);
       for(i=0;i<*d;i++){
	 fprintf(stderr,"G[%d]=",i);
	 for(j=0;j<*d;j++) fprintf(stderr,"%2ld%s",G[i][j],(*d-1==j)?"\n":" ");}
       exit(0);
     }
     return g;
}
...
{    int i,j; for(i=0;i<*d;i++) {
    for(j=0;j<*d;j++) fprintf(outFILE,"%3ld ",G[i][j]);
    fprintf(outFILE,"%s\n",s);}
}
...
{    int i,j,r, R=0,nw, p[FIB_Nmax],d[FIB_Nmax],wp[FIB_Nmax]; 
	...
	*B[AMBI_Dmax], *C[AMBI_Dmax], X[AMBI_Dmax];   if(v>AMBI_Dmax)return 0;
	...
	if(p[r]>p[R]) R=r;
	if(p[R]==p[r]) if(d[r]<d[R]) R=r;  /* p_max d_min */
     }	
     for(j=0;j<v;j++) CW->W[0][j]=W[R][j];
     CW->d[0]=d[R]; CW->nw=1;
	...
	for(i=CW->nw;i<v;i++) if(Y[i]) break;
	if(i==v) continue; 
...
{    Long vB[POLY_Dmax],vol,Z; int e,j;
     ...
     for(j=0;j<P->n;j++) printf("%s%ld",j?",":"",vB[j]);
     printf(")/%ld\n",Z);
     if(g)
...
{    int i, r=1, c=0; Long G[POLY_Dmax];
     for(i=0;i<=d;i++) if(VM[i][0]) break;
     c+=(i==d+1);    /* 0 == VM[0] */
     for(i=1;i<=d;i++) if(VM[i][1]) break;
     c+=(i==d+1); c+=(VM[0][1]==1);
...
{    int i,j, d=P->n, v=V->nv, e=E->ne, p=P->np; assert(V->nv<VERT_Nmax); 
	...
	PairMat PM; Make_VEPM(P,V,E,PM); Print_Matrix(PM, E->ne, V->nv,"PM");*/
     for(i=0;i<v;i++) P->x[V->v[i]][d]=1;
     P->n=d+1; 	V->nv++;
     for(j=0;j<=d;j++) P->x[p][j]=0;
     P->np=p+1;		V->v[v]=p;
     for(i=0;i<e;i++) {E->e[i].a[d]=E->e[i].c; E->e[i].c=0;}
     for(j=0;j<d;j++) E->e[e].a[j]=0;
     E->e[e].a[d]=-1;   E->e[e].c=1; E->ne++;
...
void EPrint_VL(PolyPointList *_P, VertexNumList *V,double f){
  ...
  for(i=0;i<_P->n;i++) {
    for(j=0;j<V->nv;j++) fprintf(stderr," %3ld",_P->x[V->v[j]][i]);
    fprintf(stderr,"\n");}
}
...
{    int e,err=0; Long VM[POLY_Dmax][VERT_Nmax]; 
  for(e=0;e<E->ne;e++)
	...
	if(err){ int i; fprintf(stderr,"%d %d  VM c=%d\n",P->n,V->nv,c);
	    for(i=0;i<P->n;i++){
	      for(j=0;j<c;j++) fprintf(stderr," %3ld",VM[i][j]);
	      fputs("",stderr);}
...
int Fano5d(PolyPointList *P,VertexNumList *V,EqList *E){  
  ...
  for(e=0;e<E->ne;e++) FI[e]=0;
  for(n=0;n<np;n++) D[n]=1;   
  for(e=0;e<E->ne;e++){Long *Y[POLY_Dmax]; int i=0;
    for(n=0;n<np;n++) if(0==Eval_Eq_on_V(&E->e[e],P->x[n],d)){
	FI[e]+=1<<n; if(i>d) return 0; p[i++]=n;}
    for(n=0;n<i;n++) Y[n]=P->x[p[n]];
  ...
  if(PrintFanoProjCand){int c; FILE *OF=outFILE; outFILE=stdout;
    ...
    for(n=26;n<125-e;n++) s[n]=s[e+n-26];
    s[n]=0; Print_PPL(P,s); outFILE=OF;
    printf("Facets[%d]:",E->ne);
    for(e=0;e<E->ne;e++){
      printf(" ");
      for(n=0;n<np;n++) printf("%d",getNI(n,FI[e]));}
    puts("");
    for(c=0;c<nc;c++){int f; printf("Circuit[%d]=",c);
      for(n=0;n<np;n++)       printf("%d",getNI(n,CI[c]));
      printf("  coeffs =");
      for(n=0;n<np;n++) if(getNI(n,CI[c])) printf(" %d",CC[c][n]);
      printf("  in");
      for(f=0;f<E->ne;f++) if((FI[f]&CI[c])==CI[c]) printf(" F[%d]",f);
      puts("");}
